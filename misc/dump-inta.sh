...
(
    ...
    grep -v "By country" | 
    grep -v "By political group" | 
    sed -e 's/^.*"\/europe\/parliament\/deputy\/\([^"]*\)">\([^<]*\)<.*$/INSERT INTO lists SET campaign=11, name="\2", url="https:\/\/memopol.lqdn.fr\/europe\/parliament\/deputy\/\1";/' \
	-e 's/^.*callto:\/\/.\([0-9]*\).*callto:\/\/.\([0-9]*\).*$/UPDATE lists SET phone="00\1" WHERE phone="";/' \
	-e 's/^.*country\/\(..\)\/.*$/UPDATE lists SET country="\1" WHERE country="";/' \
	-e 's/^.*\/group\/\([^\/]*\)\/.*$/UPDATE lists SET name=CONCAT(name," (\1)"), callduration=1 WHERE callduration=0;/'
) | mysql piphone # if necesssary : -u<user> -p<password>
...
(
    ...
    grep -v "By country" | 
    grep -v "By political group" | 
    sed -e 's/^.*"\/europe\/parliament\/deputy\/\([^"]*\)">\([^<]*\)<.*$/INSERT INTO lists SET campaign=10, name="\2", url="https:\/\/memopol.lqdn.fr\/europe\/parliament\/deputy\/\1";/' \
	-e 's/^.*callto:\/\/.\([0-9]*\).*callto:\/\/.\([0-9]*\).*$/UPDATE lists SET phone="00\1" WHERE phone="";/' \
	-e 's/^.*country\/\(..\)\/.*$/UPDATE lists SET country="\1" WHERE country="";/' \
	-e 's/^.*\/group\/\([^\/]*\)\/.*$/UPDATE lists SET name=CONCAT(name," (\1)"), callduration=1 WHERE callduration=0;/'
) | mysql piphone # if necesssary : -u<user> -p<password>
...
(
    ...
    grep -v "By country" | 
    grep -v "By political group" | 
    sed -e 's/^.*"\/europe\/parliament\/deputy\/\([^"]*\)">\([^<]*\)<.*$/INSERT INTO lists SET campaign=12, name="\2", url="https:\/\/memopol.lqdn.fr\/europe\/parliament\/deputy\/\1";/' \
	-e 's/^.*callto:\/\/.\([0-9]*\).*callto:\/\/.\([0-9]*\).*$/UPDATE lists SET phone="00\1" WHERE phone="";/' \
	-e 's/^.*country\/\(..\)\/.*$/UPDATE lists SET country="\1" WHERE country="";/' \
	-e 's/^.*\/group\/\([^\/]*\)\/.*$/UPDATE lists SET name=CONCAT(name," (\1)"), callduration=1 WHERE callduration=0;/'
) | mysql piphone # if necesssary : -u<user> -p<password>

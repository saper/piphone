...
(
    ...
    egrep "(/europe/parliament/deputy/|callto://\+32|/europe/parliament/country/|/europe/parliament/party/)" |
    grep -v "More information" | 
    grep -v "By country" | 
    grep -v "By local party" | 
    sed -e 's/^.*"\/europe\/parliament\/deputy\/\([^"]*\)">\([^<]*\)<.*$/INSERT INTO lists SET campaign=1, name="\2", url="https:\/\/memopol.lqdn.fr\/europe\/parliament\/deputy\/\1";/' \
	-e 's/^.*callto:\/\/.\([0-9]*\).*callto:\/\/.\([0-9]*\).*$/UPDATE lists SET phone="00\1" WHERE phone="";/' \
	-e 's/^.*country\/\(..\)\/.*$/UPDATE lists SET country="\1" WHERE country="";/' \
	-e 's/^.*\/party[^"]*">\([^<]*\).*$/UPDATE lists SET name=CONCAT(name," (\1)"), callduration=1 WHERE callduration=0;/'

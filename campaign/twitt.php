...
function mep_step1($localfile,$campaign) {
  ...
  while ($s=fgets($f,1024)) {
    ...
    if (preg_match('#/europe/parliament/group/([^"]*)">#',$s,$mat)) {
      $mep["group"]=substr($mat[1],0,strlen($mat[1])-1);
    }
...
function mep_step3($mep,$campaign) {
  ...
  mq("INSERT INTO lists SET 
    ...
    ;");
  */
  //  mq("UPDATE lists SET meta='".addslashes(serialize($mep))."' WHERE name='".addslashes($mep["name"])."';");
  echo $mep["name"]."    |    ".$mep["group"]."    |    ".$mep["country"]."    |    \n";
